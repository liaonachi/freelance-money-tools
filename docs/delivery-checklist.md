...
- [x] admin：錯密碼進不去；登入後發文→前台幾秒內出現；取消發布→404 且 sitemap 消失——**2026-08-22 補測完成，登入問題已找到真正原因並解決**：上一版記錄的「`ADMIN_PASSWORD` 疑似跟 Vercel 不一致」是誤判。真正原因是自動化測試 script 的 bug，不是密碼問題、也不是 Vercel env 問題：`app/admin/layout.tsx`（包住所有 `/admin/*` 路由，包含尚未登入的 `/admin/login` 頁本身）在頁首放了一個「Log out」按鈕，跟登入表單的「Log in」按鈕都是 `<button type="submit">`；測試 script 原本用 `page.click('button[type="submit"]')` 選第一個符合的按鈕，選到的其實是頁首的登出按鈕（送出 `logoutAction`，清 cookie＋導回 `/admin/login`，不管密碼對不對結果都一樣），不是登入表單自己的按鈕——本機 `npm run dev` 重現一次、對照 server log 印出 `└─ ƒ logoutAction() in 2ms` 才抓到。改成 scope 到密碼欄位所在 `<form>` 內找按鈕後，本機與正式站都一次登入成功、拿到有效 `site_admin_session` cookie（78 碼）。順手把 `ADMIN_PASSWORD` 重設成一組新的高強度隨機值（Vercel Production + 本機 `.env.local` 同步更新，未印明文，只驗證過長度與存在性），舊值失效，屬於良好衛生習慣，不代表舊值真的有問題。發文／取消發布的前後台同步（`revalidatePath` 邏輯）本身沒問題，Day 3-11 的 webhook 測試已驗證過同一條路徑。admin 文章列表截圖（`04-admin-posts.png`）已補上，5/5 張齊全
...
- **（2026-08-22）「admin 登入失敗」其實是測試 script 的 bug，不是網站的 bug**：上一輪 QA 用 `page.click('button[type="submit"]')` 測登入，選到的其實是 `app/admin/layout.tsx` 頁首「Log out」按鈕（登入頁 `/admin/login` 也被這個共用 layout 包住，未登入狀態下畫面上同時有登入表單的「Log in」跟頁首的「Log out」兩個 `type="submit"` 按鈕，選第一個就選錯）——按下去送出的是 `logoutAction`（清 cookie＋導回 `/admin/login`），跟密碼對不對完全無關，難怪怎麼試都「登入失敗」。改成 scope 到密碼欄位所在 `<form>` 再找按鈕，本機與正式站都一次登入成功。**小小的產品面 UX 瑕疵**（非本次修復範圍，記一筆給之後參考）：未登入時登入頁頁首不該出現「Log out」——`app/admin/layout.tsx` 目前對 `/admin/*` 全部路徑一視同仁渲染頁首，包含 `/admin/login` 本身；母版（`seo-tool-site-starter`）的 `app/admin/layout.tsx` 很可能有同樣的結構，如果之後要修，建議在母版修再 cherry-pick 回來。
